...
epitopeFinder = function(proj.id, e.thresh, g.method = "any", 
	...
	# proj.id = "example"; e.thresh = 1; g.method = "any"
	...
	print(paste(blast.main$qID %>% unique %>% length, "sequences to process.")) 
	
	# == == == == == Main script execution. == == == == == 
	if(autorun){
		
		# path %<>% epitopeBLAST() #for debugging single run
		
		# loop through index peptides to identify index epitopes
		path %<>% pbCycleBLAST(ncycles="max"); fwrite(glGet("blast.main"), blast.id3)
		
		# post-processing step to trim epitopes from early index peptides
		
		# path %<>% pbCycleBLAST(ncycles="max")
		# path %<>% trimEpitopes;	
		path %<>% trimEpitopes2(); fwrite(glGet("blast.main"), blast.id4)
		
		# group peptides. mode can be "any" or "all"
		indexGroups(path, mode = g.method) 
		
		#generate multiple sequence alignment motifs
		groupMSA() 
		
		#generate output table
		outputTable() 
		
		#copy relevant output files to a new directory
		outputFiles() 
	}
}

pbCycleBLAST = function(path, ncycles="max"){
	# == == == == == A. Establish start time. == == == == ==
	print(paste("pbCycleBLAST Start:", Sys.time()))
	...
	print(paste(n.pep, "peptides to process."))
	
	# == == == == == C. Run cycle of epitopeBLAST. == == == == ==
	pb <- txtProgressBar(min = -n.pep, max = 0, initial = n.pep, 
											 char = "=", width = NA, style = 3, file = "")
	
	path %<>% cycleBLAST(pb, n.pep)
	glAssign("path", path)
	
	# == == == == == D. Establish end time. == == == == ==
	# dbCleanup()
	
	cat("\n")
	print(paste("pbCycleBLAST Complete:", Sys.time()))
...
trimEpitopes2 = function(path, tofilter = FALSE){
	#for debugging
...
groupMSA = function(){
	...
	pb <- txtProgressBar(min = 1, max = num, initial = 1, 
											 char = "=", width = NA, style = 3, file = "")
	...
	for(i in 1:num){
		...
		if(length(group)>1){
			...
			# #trim groups using microseq package
			# mg <- msa(group)
			...
			cpath <- paste0("consensusSequences.txt")
			write(msaConsensusSequence(msa(group)),cpath,append=TRUE)
			...
			tname <- paste0("msa-", i, ".tex")
			pname <- paste0("msa-", i, ".pdf")
			# iname <- paste0("msa-", i, "_", 1, ".png")
			# cname <- paste0("msac.", i)
			
			# calculate figure height in inches. 
			# top margin + logo 0.75; each line 0.14;
			# consensus line 0.14, key 1.0 (prev 0.85 not enough)
			...
			# names(group) <- NULL
			names(group) <- substr(names(group),1,50)

			msaPrettyPrint(msa(group), output="tex", file = tname,
			...
			# pdf_convert(pname, format = "png", pages = NULL, filenames = NULL, 
			# 						dpi = 300, antialias = TRUE, opw = "", upw = "", 
			# 						verbose = TRUE)
...
outputFiles = function(){
	gl <- c("output.dir","proj.id","e.thresh","g.method",
					"blast.id1","blast.id2","blast.id3","blast.id4")
	for(i in gl){assign(i,glGet(i))}
	
	
	odir <- paste0(output.dir, Sys.Date(),"_",format(Sys.time(), "%H%M%S"),
								 "_", proj.id, "_epitopes/")
	bdir <- paste0(odir,"blast/")
		
	epath <- paste0(output.dir, "epitopes/", proj.id, ".fasta")
	gdir1 <- paste0(output.dir, "groups/")
	mdir1 <- paste0(output.dir, "msa/")
	opath <- paste0(output.dir, proj.id,"_e-",e.thresh,"_g-",g.method,
									"_outputTable.csv")
	...
	file.copy(epath, paste0(odir, "initial_peptides.fasta"))
	file.copy(opath, paste0(odir, proj.id, "outputTable.csv"))
	
	
	file.copy(gdir1, odir, recursive = TRUE)
	file.copy(mdir1, odir, recursive = TRUE)
	
	
	
	for(i in 1:4){
		file.copy(eval(as.name(paste0("blast.id",i))),bdir)
		file.remove(eval(as.name(paste0("blast.id",i))))
	}
	...
	fwrite(pc, paste0(odir, "parameters.txt"))
...
prepareBLAST = function(blast.thresh, tofilter=TRUE){
	...
	blast.invert <- qsSwap(blast.thresh) #flip query and subject
	blast.merge <- rbind(blast.thresh, blast.invert) %>% unique
...
epSetupDirectory = function(proj.id, e.thresh, g.method){
	...
	output.dir <- paste0("../output/", proj.id, "/")
	glAssign("output.dir", output.dir)
	if(!dir.exists(output.dir)) {dir.create(output.dir)}
	
	if(!dir.exists(paste0(output.dir, "epitopes/"))){
		dir.create(paste0(output.dir, "epitopes/"))}
	...
	glAssign("blast.id1", paste0(output.dir, proj.id, "_blast_1_raw.csv"))
	glAssign("blast.id2", paste0(output.dir, proj.id, "_blast_2_precycle.csv"))
	glAssign("blast.id3", paste0(output.dir, proj.id, "_blast_3_cycled.csv"))
	glAssign("blast.id4", paste0(output.dir, proj.id, "_blast_4_trimmed.csv"))
...
epSetupBLAST = function(){
	...
	if(file.exists(blast.id2)){
		...
		if(file.exists(blast.id1)){
			...
			fwrite(blast.path, blast.id1) #write ~precomputed blast to csv 
		}
...
decipherGaps = function(blast){
	...
	pb <- txtProgressBar(min = 0, max = length(gpos), initial = 0, 
											 char = "=", width = NA, style = 3, file = "")
	pbcount <- 0
	...
	for(i in gpos){
		...
		m <- msa(blast[i, c(qSeq, sSeq)] %>% as.character %>% AAStringSet)
		c <- m %>% as.character
		if(nchar(c[1]) != nchar(c[2])){stop("ERROR: different nchar deciperGaps")}
		...
		for(j in 2:nchar(c[1])){ #loop through remaining & make a note of switch
			if(j %in% g){
			...
			if((j == nchar(c[1])) & (gap == FALSE)){e <- c(e, j)}
		}
		
		aln <- data.frame(cbind(s, e))
		# aln <- aln[(aln$e-aln$s)>5, ] #remove small
		...
		for(j in 1:nrow(aln)){
			...
			g.q <- gregexpr("-", substr(c[1], 1, aln[j, 1])) %>% unlist
			g.s <- gregexpr("-", substr(c[2], 1, aln[j, 1])) %>% unlist
